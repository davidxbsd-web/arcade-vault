...
import { GAMES, PLAYERS, seededScores } from "@/lib/data";
...
export default function JuegosPage() {
  ...
  const ticker = seededScores(410, 7).map((r, i) => ({
    ...r,
    game: GAMES[i % GAMES.length].title,
    time: TICKER_TIMES[i] ?? `hace ${(i + 1) * 5} min`,
    color: TICKER_COLORS[i % TICKER_COLORS.length],
  }));

  const topPlayers = seededScores(730, Math.min(5, PLAYERS.length));
      ...
      <section className="home-stats reveal">
        <div className="stats-inner">
          <div className="stat-block">
            <div className="stat-n neon-yellow">{GAMES.length}+</div>
            <div className="stat-u pixel">JUEGOS</div>
            <div className="stat-s">Y CONTANDO</div>
          </div>
          <div className="stat-block">
            <div className="stat-n neon-yellow">MILES</div>
            <div className="stat-u pixel">DE PARTIDAS</div>
            <div className="stat-s">JUGADAS CADA DÍA</div>
          </div>
          <div className="stat-block">
            <div className="stat-n neon-yellow">GLOBAL</div>
            <div className="stat-u pixel">RANKING</div>
            <div className="stat-s">COMPITE CON EL MUNDO</div>
          </div>
        </div>
      </section>
